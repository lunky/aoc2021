...
day4 input = head (snd winner) * sum (concat (fst winner) \\ snd winner)
  where 
    ...
    winner = head $ filter isWinner $ head $ dropWhile (not . any isWinner ) $ scanl (flip  callNumber) boards bingoNumbers
...
day4b' input =  winner
  where 
    ...
    winner = head 
               $ filter isLast 
               $ head 
               $ dropWhile (not . all isWinner) 
               $ scanl (flip callNumber) boards bingoNumbers
...
readBoards input =  chunksOf 5 $ map (map (\x->read x::Int) . words) $ filter (/="") input

updateBoardState (board,seen) next 
  | elem next $ concat board = (board, next:seen)
  | otherwise = (board,seen)

isWinner :: (Foldable t, Eq a) => ([[a]], t a) -> Bool
isWinner (board,seen) = 
      any (all (`elem` seen)) (board ++ transpose board)

isLast :: Eq a => ([[a]], [a]) -> Bool
isLast (board,seen) = 
      any (\y -> (head seen `elem` y )  && all (`elem` seen) y) (board ++ transpose board)

callNumber number = map (`updateBoardState` number)

initializeBoards = map (,[])
